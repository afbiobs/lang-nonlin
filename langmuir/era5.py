"""ERA5/Open-Meteo download helpers for validation workflows."""
...
ERA5_REQUEST_TIMEOUT_S = 45
ERA5_MAX_RETRIES = 4
ERA5_BACKOFF_BASE_S = 1.5
ERA5_HOURLY_VARS = [
    "wind_speed_10m",
    "wind_direction_10m",
    "wind_gusts_10m",
...
def download_era5(
    ...
    for attempt in range(1, ERA5_MAX_RETRIES + 1):
        try:
            with urlopen(url, timeout=ERA5_REQUEST_TIMEOUT_S) as response:
            ...
            if attempt == ERA5_MAX_RETRIES:
                raise RuntimeError(
                    f"ERA5 download failed for ({lat}, {lon}) {start_date} to {end_date} "
                    f"after {ERA5_MAX_RETRIES} attempts: {exc}"
                ) from exc

            backoff_s = ERA5_BACKOFF_BASE_S * (2 ** (attempt - 1)) + random.uniform(0.0, 0.5)
            time.sleep(backoff_s)
    else:
        raise RuntimeError(
            f"ERA5 download failed for ({lat}, {lon}) {start_date} to {end_date}: {last_error}"
        )
